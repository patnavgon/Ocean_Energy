...
var SearchableMapLib = {
  ...
  initialize: function(options){
    ...
    if (!SearchableMapLib.map) {
      ...
      // Custom resource legend-----
      var legend = L.control({position: 'bottomright'});

      legend.onAdd = function (map) {
        var div = L.DomUtil.create('div', 'info legend');
         var statuses = [
            { label: "Salinity gradient", icon: "marker-icon-2x-red.png" },
            { label: "Tidal", icon: "marker-icon-2x-blue.png" },
            { label: "Wave", icon: "marker-icon-2x-green.png" },
            { label: "Other", icon: "marker-icon-2x-yellow.png" }
          ];
        
        div.innerHTML += '<h4 style="font-size:13px;">Resource</h4>';

        statuses.forEach(function(status) {
           div.innerHTML +=
            `<div style="margin-bottom: 4px;">
             <img src="img/${status.icon}" style="height: 20px; vertical-align: middle; margin-right: 5px;">
              ${status.label}
            </div>`;
          });
    ...
    // End resource legend-----
legend.addTo(SearchableMapLib.map);
  ...
  createSQL: function() {
    ...
    var resource_types = $(".resource-filter:checked").map(function() {
      return $(this).val();
    }).get();
    
    if (resource_types.length > 0) {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return resource_types.includes(r.properties["RESOURCE"]);
      });
    }
    // -----end resource filter-----
    // -----custom scale filter-----
    var scale_types = $("#scale-filter option:selected").map(function() {
      return $(this).val();
    }).get().filter(val => val !== '');
    
    if (scale_types.length > 0) {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return scale_types.includes(r.properties["PROJESCALE"]);
      });
    }
    // -----end scale filter-----

    // -----custom status filter-----
    var status_types = $("#status-filter option:selected").map(function() {
      return $(this).val();
    }).get().filter(val => val !== '');
    
    if (status_types.length > 0) {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
        return status_types.includes(r.properties["STATUS"]);
      });
    }
    // -----end status filter-----
    ...
    SearchableMapLib.currentResultsLayer = L.geoJSON(SearchableMapLib.currentResults, {
        pointToLayer: function (feature, latlng) {
          return L.marker(latlng, {icon: SearchableMapLib.getIcon(feature.properties["RESOURCE"])} );
        },
